...
public:
    ...
    KNNEnsembleOptm(const Data<T> &data, size_t k, double alpha, bool mult_accs = false, size_t folds=10, size_t seed=42, int verbose = 0):
...
#pragma unroll
        for (size_t j = 0; j < n_learners; j++) {
            Data<T> data = this->samples->selectFeatures(subspaces[j]);

            auto classifier = dynamic_cast<classifier::Classifier<T> *>(this->m_learners[j].get());
            auto report = validation::kkfold(data, *classifier, 10, 10, this->seed, 0);
            accs[j] = report.accuracy / 100.0;
        }
        for(size_t i = 0; i < kfold_splits.size(); i++) {
            mltk::Data<double> train = kfold_splits[i].train;
...
#else
            int learner_pos = 0;
            std::transform(this->m_learners.begin(), this->m_learners.end(), results.begin(), [this, &train, test, &learner_pos]
                    (auto& learner){
                arma::colvec preds(test.size(), arma::fill::zeros);

                mltk::Data<double> train_data = train.selectFeatures(subspaces[learner_pos]);
                learner->setSamples(train_data);
                learner->train();

                for(int i = 0; i < test.size(); i++){
                    preds(i) = class_maper[learner->evaluate(test(i).selectFeatures(subspaces[learner_pos]))];
