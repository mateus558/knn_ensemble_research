...
int main(){
        ...
        for(const auto alpha: alphas) {
            ...
            std::for_each(std::execution::par_unseq, orders.begin(), orders.end(), [&max_acc, &log, &_patience, patience,
               ...
               if(max_acc == 100.0 || _patience <= 0) return;
                mltk::ensemble::kNNEnsembleWPDSM<double> knn(data, 7, alpha, order);
                knn.set_test(true);
    ...
    for(const auto& dataset: log){
        std::ofstream ofile("max_acc_log/max_acc_k_7_"+dataset.first+"_log.csv");
        if(!ofile.is_open()){
